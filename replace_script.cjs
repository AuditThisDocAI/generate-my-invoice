...
files.forEach(file => {
    ...
    if (content.includes('Workspace Eazy') || content.includes('Workspace eazy') || content.includes('WORKSPACE EAZY')) {
    ...
    if (content.includes('WORKSPACE') && content.includes('EAZY')) {
    ...
    if (content.includes('Aria Assistant') || content.includes('Aria assistant')) {
         content = content.replace(/Aria Assistant/g, 'Aria Workflow Assistant');
         content = content.replace(/Aria assistant/g, 'Aria Workflow assistant');
         changed = true;
    }

    if (content.includes('Aria Workflow Assistant') && content.includes('Aria Workflow Workflow Assistant')) {
       // Just in case we double replaced
       content = content.replace(/Aria Workflow Workflow Assistant/g, 'Aria Workflow Assistant');
       changed = true;
    }

    // Replace the name of the app in i18n
    if (content.includes('Workspace Eazy') || content.includes('Aria Workflow')) {
        // Just in case
    }
